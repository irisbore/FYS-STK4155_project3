...
if __name__ == "__main__":
    ...
    for batch_size in batch_sizes:
        val_accuracies = run_cv(trainset, config, epochs=10, learning_rate=0.01, batch_size=batch_size)
        mean_accuracy = float(np.mean(val_accuracies))
        std_accuracy = float(np.std(val_accuracies))
    ...
    utils.plot_parameter_study(batch_sizes, batch_accuracies, "Batch Size")
